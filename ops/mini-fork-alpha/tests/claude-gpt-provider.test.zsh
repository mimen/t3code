...
/usr/bin/grep -qx 'MINI_FORK_ALPHA_CLAUDE_CLI_PATH="/Users/REPLACE_ME/.local/bin/claude"' "$config_example"
/usr/bin/grep -qx 'MINI_FORK_ALPHA_CLAUDE_CLI_SHA256="REPLACE_WITH_64_LOWERCASE_HEX_CHARACTERS"' "$config_example"
if /usr/bin/grep -qF 'MINI_FORK_ALPHA_CLAUDE_GPT_CLI_PATH=' "$config_example"; then
  print -u2 -r -- "Mini config must point to the actual Claude CLI, not claude-gpt."
  exit 1
fi

/usr/bin/grep -qFx '  --gateway-key-fd 0' "$wrapper"
/usr/bin/grep -qF '"$MINI_FORK_ALPHA_NODE_BIN" "$release/apps/server/dist/bin.mjs" mini configure-claude-gpt' "$wrapper"
/usr/bin/grep -qF 'release_for_link "$MINI_FORK_ALPHA_ROOT/current"' "$wrapper"
...
/usr/bin/grep -qF 'canonical_path "$MINI_FORK_ALPHA_CLAUDE_CLI_PATH"' "$wrapper"
/usr/bin/grep -qF '/usr/bin/shasum -a 256 "$canonical_claude_cli_path"' "$wrapper"
/usr/bin/grep -qF 'MINI_FORK_ALPHA_CLAUDE_CLI_SHA256' "$wrapper"
/usr/bin/grep -qF -- '--base-dir "$MINI_FORK_ALPHA_DATA_DIR"' "$wrapper"
/usr/bin/grep -qF -- '--claude-cli-path "$canonical_claude_cli_path"' "$wrapper"
/usr/bin/grep -qF -- '--claude-cli-sha256 "$MINI_FORK_ALPHA_CLAUDE_CLI_SHA256"' "$wrapper"
/usr/bin/grep -qF -- '--claude-home-path "$MINI_FORK_ALPHA_CLAUDE_GPT_HOME_PATH"' "$wrapper"
