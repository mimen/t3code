...
/usr/bin/grep -qx 'MINI_FORK_STAGING_PORT="8447"' "$ops_root/config.example.zsh"
/usr/bin/grep -qx 'MINI_FORK_STAGING_HOST="127.0.0.1"' "$ops_root/config.example.zsh"
/usr/bin/grep -qx 'MINI_FORK_STAGING_ELIGIBLE_REF="refs/heads/mini-fork-staging/eligible"' "$ops_root/config.example.zsh"
/usr/bin/grep -qx 'MINI_FORK_STAGING_ELIGIBILITY_ALLOWED_SIGNERS_PATH="/Users/REPLACE_ME/.config/t3code-fork-staging/eligibility-allowed-signers"' "$ops_root/config.example.zsh"
/usr/bin/grep -qx 'MINI_FORK_STAGING_SSH_KEYGEN_BIN="/usr/bin/ssh-keygen"' "$ops_root/config.example.zsh"
/usr/bin/grep -qx 'MINI_FORK_STAGING_SERVER_PLIST_PATH="/Users/REPLACE_ME/Library/LaunchAgents/com.mimen.t3code.fork-staging.plist"' "$ops_root/config.example.zsh"
...
replacements = {
    "__OPS_ROOT__": "/Users/mini/Library/Application Support/t3code-fork-staging/ops/mini-fork-staging",
    "__CONFIG_PATH__": "/Users/mini/.config/t3code-fork-staging/config.zsh",
    "__ROOT__": "/Users/mini/Library/Application Support/t3code-fork-staging",
    "__LOG_DIR__": "/Users/mini/Library/Logs/t3code-fork-staging",
    "__POLL_INTERVAL_SECONDS__": "300",
}
templates = {
    "com.mimen.t3code-fork-staging.server.plist.template": [
        "com.mimen.t3code.fork-staging",
    ...
    "com.mimen.t3code-fork-staging.poll.plist.template": [
        "com.mimen.t3code.fork-staging.poll",
...
/usr/bin/grep -qx '"$MINI_FORK_STAGING_GIT" clone --no-checkout --no-local "$mirror" "$temporary_release" >/dev/null 2>&1 || fail "Could not create staging checkout."' "$ops_root/scripts/stage.zsh"
...
/usr/bin/grep -qFx '  [[ "$MINI_FORK_STAGING_HOST" == "127.0.0.1" ]] || fail "MINI_FORK_STAGING_HOST must be 127.0.0.1."' "$ops_root/scripts/lib.zsh"
/usr/bin/grep -qF -- '-iTCP@"$MINI_FORK_STAGING_HOST":"$MINI_FORK_STAGING_PORT"' "$ops_root/scripts/lib.zsh"
...
/usr/bin/grep -qx 'permissions:' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '  contents: read' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '  publish_eligible_ref:' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx "    if: github.event_name == 'push' && github.ref == 'refs/heads/fork-staging'" "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '      GITHUB_TOKEN: ${{ github.token }}' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '      MINI_STAGING_ELIGIBILITY_SIGNING_KEY: ${{ secrets.MINI_STAGING_ELIGIBILITY_SIGNING_KEY }}' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '      contents: write' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '    needs: verify' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qFx '          if [[ -z "${MINI_STAGING_ELIGIBILITY_SIGNING_KEY:-}" ]]; then' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qFx '          [[ "$signer_public_key" == ssh-ed25519\ * ]] || {' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qFx '          if [[ "$current_staging_sha" != "$GITHUB_SHA" ]]; then' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '          ssh-keygen -Y sign -f "$signing_key" -n mini-fork-staging-eligibility "$payload_path" >/dev/null' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '          git -C "$publish_dir" push --force origin HEAD:refs/heads/mini-fork-staging/eligible' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx '        uses: actions/checkout@d23441a48e516b6c34aea4fa41551a30e30af803 # v6' "$repo_root/.github/workflows/mini-fork-staging-eligibility.yml"
/usr/bin/grep -qx 'export T3CODE_PORT="8447"' "$ops_root/scripts/run-server.zsh"
/usr/bin/grep -qx 'export T3CODE_TAILSCALE_SERVE="false"' "$ops_root/scripts/run-server.zsh"
/usr/bin/grep -qx '  --port 8447 \\' "$ops_root/scripts/run-server.zsh"
/usr/bin/grep -qF '## Required eligibility signing setup' "$ops_root/README.md"
/usr/bin/grep -qF 'MINI_STAGING_ELIGIBILITY_SIGNING_KEY' "$ops_root/README.md"
/usr/bin/grep -qF 'MINI_FORK_STAGING_ELIGIBILITY_ALLOWED_SIGNERS_PATH' "$ops_root/README.md"
/usr/bin/grep -qF 'The eligibility ref need not be protected' "$ops_root/README.md"
/usr/bin/grep -qF '127.0.0.1:8447' "$ops_root/README.md"
/usr/bin/grep -qF 'Tailnet-IP `:8447` listener separately' "$ops_root/README.md"

print -r -- "Mini Fork Staging deployment eligibility checks passed."
