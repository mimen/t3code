#!/bin/zsh
# LaunchAgent entrypoint. It starts only the current immutable release on port 8447.
...
export T3CODE_PORT="8447"
export T3CODE_HOST="$MINI_FORK_STAGING_HOST"
export T3CODE_HOME="$MINI_FORK_STAGING_DATA_DIR"
...
exec "$MINI_FORK_STAGING_NODE_BIN" "$release/apps/server/dist/bin.mjs" serve \
  --port 8447 \
  --host "$MINI_FORK_STAGING_HOST" \
