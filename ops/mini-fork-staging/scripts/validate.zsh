...
endpoint="http://${MINI_FORK_STAGING_HOST}:8447/.well-known/t3/environment"
descriptor_path="$MINI_FORK_STAGING_ROOT/state/descriptor.$$.json"
cleanup_descriptor() {
...
while (( attempt <= 30 )); do
  if port_is_owned_by_release "$release" &&
    "$MINI_FORK_STAGING_CURL" --fail --silent --show-error --connect-timeout 2 --max-time 4 "$endpoint" > "$descriptor_path" 2>/dev/null; then
    if "$MINI_FORK_STAGING_PYTHON" - "$descriptor_path" "$expected_version" <<'PYTHON'
import json
import sys
