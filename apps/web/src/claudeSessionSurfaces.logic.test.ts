...
describe("federated catalogue queries", () => {
  it("removes every desktop-local environment in Remote-only mode", () => {
    const laptop = "laptop" as FederatedClaudeSession["environmentId"];
    const localSecondary = "local-secondary" as FederatedClaudeSession["environmentId"];
    const mini = "mini" as FederatedClaudeSession["environmentId"];
    const environments = [
      { environmentId: laptop, label: "Laptop" },
      { environmentId: localSecondary, label: "Local secondary" },
      { environmentId: mini, label: "Mini" },
    ];

    expect(
      filterClaudeSessionEnvironments(environments, true, new Set([laptop, localSecondary])),
    ).toEqual([{ environmentId: mini, label: "Mini" }]);
    expect(filterClaudeSessionEnvironments(environments, false, new Set([laptop]))).toBe(
      environments,
    );
  });
