...
export function ClaudeSessionBrowserDialog({
  ...
  const visibleEnvironments = useMemo(
    () =>
      filterClaudeSessionEnvironments(environments, remoteOnlyDesktop, desktopLocalEnvironmentIds),
    [desktopLocalEnvironmentIds, environments, remoteOnlyDesktop],
  );
  const visibleEnvironmentIds = useMemo(
    () => new Set(visibleEnvironments.map((environment) => environment.environmentId)),
    [visibleEnvironments],
  );
  const visibleProjectScope = useMemo(
    () =>
      projectScope === undefined || projectScope === null
        ? projectScope
        : {
            ...projectScope,
            locations: projectScope.locations.filter((location) =>
              visibleEnvironmentIds.has(location.environmentId),
            ),
          },
    [projectScope, visibleEnvironmentIds],
  );
  ...
  const queriesByEnvironment = useMemo(
    () =>
      isProjectScoped
        ? buildProjectClaudeSessionQueries({ scope: visibleProjectScope ?? null, filters })
        : buildGlobalClaudeSessionQueries({
            environmentIds: visibleEnvironments.map((environment) => environment.environmentId),
            selection: environmentSelection,
            filters,
          }),
    [environmentSelection, filters, isProjectScoped, visibleEnvironments, visibleProjectScope],
  );
  const exactQueriesByEnvironment = useMemo(
    () => buildExactClaudeSessionQueries({ scope: visibleProjectScope ?? null, filters }),
    [filters, visibleProjectScope],
  );
  ...
  const exactCatalogue = useClaudeSessionCatalogue({
    enabled: open && isProjectScoped && visibleProjectScope !== null,
    environments: visibleEnvironments,
  ...
  const projectSessions = useMemo(
    () =>
      splitClaudeSessionsForProjectScope({
        ...
        scope: visibleProjectScope ?? null,
      }),
    [catalogue.sessions, visibleProjectScope],
  );
  ...
  const showEnvironment = isProjectScoped
    ? new Set(visibleProjectScope?.locations.map((location) => location.environmentId) ?? []).size >
      1
    : environmentSelection === ALL_CLAUDE_SESSION_ENVIRONMENTS;
  const exactCwdLabels = visibleProjectScope?.locations.map((location) => location.exactCwd) ?? [];

  useEffect(() => {
    if (
      environmentSelection !== ALL_CLAUDE_SESSION_ENVIRONMENTS &&
      !visibleEnvironmentIds.has(environmentSelection)
    ) {
      setEnvironmentSelection(ALL_CLAUDE_SESSION_ENVIRONMENTS);
    }
  }, [environmentSelection, visibleEnvironmentIds]);
