...
> {
  ...
  const allModels = catalog.pendingModels();
  ...
  const models = catalog.modelsForVersion(parsedVersion);
  const versionUpgradeMessage = catalog.versionUpgradeMessage(parsedVersion);
...
export const makePendingClaudeProvider = (
  ...
  Effect.gen(function* () {
    ...
    const models = catalog.pendingModels();
